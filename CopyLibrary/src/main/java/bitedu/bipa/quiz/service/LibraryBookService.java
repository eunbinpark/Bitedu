...
public class LibraryBookService {
	...
	// select * from book_use_status where user_id = 'user1' and borrow_start
	// between '2023-6-1' and '2023-6-30';
	...
	private ArrayList<BookUseStatusVO> getNotReturnedBooks(ArrayList<BookUseStatusVO> bookList, Calendar criteriaDate) {
		...
		for (BookUseStatusVO book : bookList) {
			if (book.getBorrowEnd().getTime() < criteriaDate.getTimeInMillis() && book.getReturnDate() == null) {
				...
				Timestamp temp = DateCalculation.calcuStopDate(new Timestamp(criteriaDate.getTimeInMillis()),
						book.getBorrowEnd());
	...
	private ArrayList<BookUseStatusVO> getExpectingReturnBooks(ArrayList<BookUseStatusVO> bookList,
			Calendar criteriaDate) {
		ArrayList<BookUseStatusVO> result = null;
		// 반납예정도서
		result = new ArrayList<BookUseStatusVO>();
		for (BookUseStatusVO book : bookList) {
	...
	public boolean rent_Book(String userId, int bookNum) {
		boolean flag = false;
		boolean result = false;
		flag = dao.rent_Book(bookNum);

		LocalDate now = LocalDate.now();
		Date nowdate = java.sql.Date.valueOf(now);
		List<Date> borrowEndDates = dao.getBorrowEndDates(userId);
		for (Date borrowEndDate : borrowEndDates) {

			if (nowdate.after(borrowEndDate)) {
				System.out.println("대출 불가능");
			}
		}

		if (flag) {
			if (dao.rent_Book_commit(userId, bookNum)) {
				result = true;
			} else {
	...
	public boolean return_book(String userId, int bookNum) {
		boolean flag = false;
		flag = dao.return_book(bookNum, userId);
