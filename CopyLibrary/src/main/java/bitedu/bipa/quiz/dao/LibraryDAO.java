...
public class LibraryDAO {
	...
	public ArrayList<BookUseStatusVO> selectBookInfoByUser(String userId, String startMonth) throws SQLException {
		...
		while (rs.next()) {
			vo = new BookUseStatusVO(rs.getInt(4), rs.getString(5), rs.getTimestamp(6), rs.getTimestamp(7),
					rs.getTimestamp(8));
	...
	public String get_bookisbn(int index) {
		Connection con = manager.getConnection();
		String sql = new StringBuilder().append("select book_isbn ").append("from book_copy ")
				.append("where book_seq = ?").toString();
		String result = null;
		try {
			PreparedStatement pstmt = con.prepareStatement(sql);
	...
	// 책 대출 book_info 테이블과 book_copy 테이블 조인해서 책 상태 확인하고 해당 인덱스 번호에 해당하는 책 대출 가능한지 ->
	// 책 상태가 BM-0001이면 대출가능 아니면 대출불가로 구분
	public boolean rent_Book(int index) {
		...
		String sql = new StringBuilder().append(
				"select a.book_isbn, book_title, book_author, book_published_date, book_seq, book_position, book_status ")
				.append("from book_info a ").append("join book_copy b ").append("on ? = b.book_isbn;").toString();
		try {
			PreparedStatement pstmt = con.prepareStatement(sql);
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				BVO = new BookVO(rs.getInt("book_seq"), rs.getString("book_position"), rs.getString("book_status"),
	...
	public boolean rent_Book_commit(String UserID, int bookNum) {
		...
		String sql = new StringBuilder().append(
				"INSERT INTO book_use_status (book_seq, user_id, borrow_start, borrow_end) VALUES (?, ?, ?, ?);")
				.toString();
	...
	public boolean return_book(int index, String ID) {
		boolean flag = false;
		// UPDATE book_use_status SET return_date = date_format(NOW(),'%Y-%m-%d') WHERE
		// user_id = 'user1' AND book_seq = 10;
		String sql = new StringBuilder().append("UPDATE book_use_status ")
				.append("SET return_date = date_format(NOW(),'%Y-%m-%d') ").append("WHERE user_id = ? AND book_seq = ?")
				.toString();
		try {
			Connection con = manager.getConnection();
			PreparedStatement pstmt = con.prepareStatement(sql);
	...
	public UserVO getUSerVO(String ID) {
		PreparedStatement pstmt = null;
		String sql = new StringBuilder().append("select * ").append("from book_user ").append("where user_id = ? ")
				.toString();
		UserVO UVO = null;
		try {
			Connection con = manager.getConnection();
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				UVO = new UserVO(rs.getInt("user_seq"), rs.getString("user_id"), rs.getString("user_pass"),
						rs.getString("user_phone_number"), rs.getString("user_status"), rs.getString("user_grade"),
	 ...
	 public List<Date> getBorrowEndDates(String userId) {
         List<Date> borrowEndDates = new ArrayList<>();

         String selectQuery = "SELECT borrow_end FROM book_use_status WHERE user_id = ?";
         try {
            Connection con = manager.getConnection();
             ...
             while (resultSet.next()) {
                 Date borrowEndDate = resultSet.getDate("borrow_end");
                 borrowEndDates.add(borrowEndDate);
             }
