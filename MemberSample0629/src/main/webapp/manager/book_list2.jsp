...
<body>
	...
	<table>
		...
		<tr>
			...
			<td><a href='./BlmController?cmd=bookdetail&bookSeq=<%=copy.getBookSeq()%>'><%=copy.getTitle()%></a></td>
			<td><%=copy.getAuthor()%></td>
			<td><%=copy.getPublishDate()%></td>
			<td><a
				href="./BlmController?cmd=remove&bookSeq=<%=copy.getBookSeq()%>">삭제</a></td>
		</tr>
		<%
		}
		%>
